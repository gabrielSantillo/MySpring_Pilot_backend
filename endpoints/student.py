...
def patch():
    ...
    if(type(student_info) == list and len(student_info) != 0):
        update_student_info = check_data_sent(request.json, student_info[0], ['student_id', 'college_id', 'english', 'app_form', 'comission'])

        results = run_statement('CALL edit_student(?,?,?,?,?,?)', [update_student_info['student_id'], update_student_info['college_id'], update_student_info['english'], update_student_info['app_form'], update_student_info['comission'], request.headers.get('token')])
